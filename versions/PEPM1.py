...
def load_plaintext_settings():
    ...
    try:
        ...
        try:
            argon2_params["time_cost"] = int(
                settings.get("time_cost", argon2_params["time_cost"])
            )
            argon2_params["memory_cost"] = int(
                settings.get("memory_cost", argon2_params["memory_cost"])
            )
            argon2_params["parallelism"] = int(
                settings.get("parallelism", argon2_params["parallelism"])
            )
            argon2_params["hash_len"] = int(
                settings.get("hash_len", argon2_params["hash_len"])
            )
            argon2_params["salt_len"] = int(
                settings.get("salt_len", argon2_params["salt_len"])
            )
        except Exception:
            logger.debug(
                "Could not parse some Argon2 params from plaintext settings; using defaults."
            )
    ...
    try:
        ...
        try:
            salt_file.write_text(
                base64.b64encode(salt).decode("utf-8"), encoding="utf-8"
            )
...
def load_from_json():
    ...
    if not json_file.exists():
        messagebox.showinfo(
            "Info", f"{json_file} not found. A new file will be created."
        )
        data = []
        try:
            save_to_json()
...
def main():
    ...
    while True:
        ...
        retry = messagebox.askretrycancel(
            "Access denied",
            "Incorrect password or decryption failed. Retry?",
            parent=root,
        )
        if not retry:
            root.destroy()
    ...
    header = tk.Label(
        root, text="Password Manager", font=("Segoe UI", 16), bg=bg_color, fg=fg_color
    )
    header.pack(pady=8)
    ...
    listbox = tk.Listbox(
        list_frame, activestyle="none", bg=entry_bg, fg=fg_color, selectmode=tk.SINGLE
    )
    ...
    tk.Label(form, text="Context:", bg=bg_color, fg=fg_color).grid(
    ...
    tk.Label(form, text="Username:", bg=bg_color, fg=fg_color).grid(
    ...
    tk.Label(form, text="Password:", bg=bg_color, fg=fg_color).grid(
    ...
    def refresh_listbox():
        ...
        for idx, item in enumerate(data):
            listbox.insert(
                tk.END,
                f"{idx + 1}. {item.get('context', '')} — {item.get('user', '')} — {item.get('password', '')}",
            )
    ...
    def on_add():
        ...
        if editing_state["is_editing"] and editing_state["edit_index"] is not None:
            ...
            data.append(
                {"context": context, "user": username, "password": password_val}
            )
    ...
    def on_settings():
        ...
        tk.Label(params_frame, text="Time Cost:", bg=bg_color, fg=fg_color).grid(
            row=0, column=0, sticky="e", padx=5, pady=5
        )
        time_cost_var = tk.StringVar(value=str(argon2_params["time_cost"]))
        tk.Entry(
            params_frame,
            textvariable=time_cost_var,
            width=20,
            bg=entry_bg,
            fg=fg_color,
            insertbackground=fg_color,
        ...
        tk.Label(params_frame, text="Memory Cost (KB):", bg=bg_color, fg=fg_color).grid(
            row=1, column=0, sticky="e", padx=5, pady=5
        )
        memory_cost_var = tk.StringVar(value=str(argon2_params["memory_cost"]))
        tk.Entry(
            params_frame,
            textvariable=memory_cost_var,
            width=20,
            bg=entry_bg,
            fg=fg_color,
            insertbackground=fg_color,
        ...
        tk.Label(params_frame, text="Parallelism:", bg=bg_color, fg=fg_color).grid(
            row=2, column=0, sticky="e", padx=5, pady=5
        )
        parallelism_var = tk.StringVar(value=str(argon2_params["parallelism"]))
        tk.Entry(
            params_frame,
            textvariable=parallelism_var,
            width=20,
            bg=entry_bg,
            fg=fg_color,
            insertbackground=fg_color,
        ...
        tk.Label(
            params_frame, text="Salt Length (bytes):", bg=bg_color, fg=fg_color
        ).grid(row=3, column=0, sticky="e", padx=5, pady=5)
        salt_len_var = tk.StringVar(value=str(argon2_params["salt_len"]))
        tk.Entry(
            params_frame,
            textvariable=salt_len_var,
            width=20,
            bg=entry_bg,
            fg=fg_color,
            insertbackground=fg_color,
        ...
        tk.Label(params_frame, text="New Password:", bg=bg_color, fg=fg_color).grid(
            row=4, column=0, sticky="e", padx=5, pady=5
        )
        app_pass_var = tk.StringVar(value="")
        tk.Entry(
            params_frame,
            textvariable=app_pass_var,
            show="*",
            width=20,
            bg=entry_bg,
            fg=fg_color,
        ...
        tk.Label(
            params_frame, text="Confirm New Password:", bg=bg_color, fg=fg_color
        ).grid(row=5, column=0, sticky="e", padx=5, pady=5)
        app_confirm_var = tk.StringVar(value="")
        tk.Entry(
            params_frame,
            textvariable=app_confirm_var,
            show="*",
            width=20,
            bg=entry_bg,
            fg=fg_color,
        ...
        def regenerate_salt():
            ...
            try:
                ...
                messagebox.showerror(
                    "Error", f"Failed to re-save data after salt regeneration: {e}"
                )
                return
    ...
    status = tk.Label(
        root, text="Ready", bd=1, relief="sunken", anchor="w", bg=button_bg, fg=fg_color
    )
